...
export async function findBestSuperStakeIxs({
	...
	if (marketIndex === 2) {
		...
		return findBestLstSuperStakeIxs({
			amount,
			lstMint: dainClient.getSpotMarketAccount(8).mint,
			lstMarketIndex: 8,
			jupiterClient,
			dainClient,
			userAccountPublicKey,
...
export async function findBestMSolSuperStakeIxs({
	...
	if (!price) {
		const marinadeProgram = getMarinadeFinanceProgram(dainClient.provider);
		price = await getMarinadeMSolPrice(marinadeProgram);
	}

	const solSpotMarketAccount = dainClient.getSpotMarketAccount(1);
	const mSolSpotMarketAccount = dainClient.getSpotMarketAccount(2);
	...
	if (!jupiterPrice || price <= jupiterPrice || forceMarinade) {
		const ixs = await dainClient.getStakeForMSOLIx({
			amount,
			userAccountPublicKey,
		});
		return {
		...
		const { ixs, lookupTables } = await dainClient.getJupiterSwapIxV6({
			inMarketIndex: 1,
			outMarketIndex: 2,
			jupiterClient,
			amount,
...
export async function findBestJitoSolSuperStakeIxs({
	...
	return await findBestLstSuperStakeIxs({
		...
		userAccountPublicKey,
		onlyDirectRoutes,
		lstMint: dainClient.getSpotMarketAccount(6).mint,
		lstMarketIndex: 6,
		jupiterQuote,
	});
}
