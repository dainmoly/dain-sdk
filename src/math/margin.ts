...
export function calculateMarginUSDCRequiredForTrade(
	...
	const targetMarket = dainClient.getPerpMarketAccount(targetMarketIndex);

	const price =
		entryPrice ??
		dainClient.getOracleDataForPerpMarket(targetMarket.marketIndex).price;
...
export function calculateCollateralDepositRequiredForTrade(
	...
	const collateralMarket = dainClient.getSpotMarketAccount(collateralIndex);

	const collateralOracleData =
		dainClient.getOracleDataForSpotMarket(collateralIndex);
	...
	const baseAmountRequired = dainClient
