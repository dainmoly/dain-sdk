...
export type NetworkEnv = 'devnet' | 'mainnet-beta';
...
export const configs: { [key in NetworkEnv]: DainConfig } = {
	devnet: {
		ENV: 'devnet',
...
let currentConfig: DainConfig = configs.devnet;

export const getConfig = (): DainConfig => currentConfig;
...
export const initialize = (props: {
	env: NetworkEnv;
	overrideEnv?: Partial<DainConfig>;
}): DainConfig => {
	//@ts-ignore
	if (props.env === 'master')
