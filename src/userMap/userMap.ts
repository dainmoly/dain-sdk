...
export class UserMap implements UserMapInterface {
  ...
  public async subscribe() {
    ...
    await this.dainClient.subscribe();
    this.lastNumberOfSubAccounts =
      this.dainClient.getStateAccount().numberOfSubAccounts;
    if (!this.disableSyncOnTotalAccountsChange) {
      this.dainClient.eventEmitter.on(
        "stateAccountUpdate",
  ...
  private async defaultSync() {
    ...
    try {
      ...
      const rpcRequestArgs = [
        this.dainClient.program.programId.toBase58(),
        {
          commitment: this.commitment,
          filters,
  ...
  public async unsubscribe() {
    ...
    if (this.lastNumberOfSubAccounts) {
      if (!this.disableSyncOnTotalAccountsChange) {
        this.dainClient.eventEmitter.removeListener(
          "stateAccountUpdate",
          this.stateAccountUpdateCallback
        );
      }
