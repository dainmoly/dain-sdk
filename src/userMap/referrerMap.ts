...
export class ReferrerMap {
	 ...
	 *
	 * @param {DainClient} dainClient - The DainClient instance.
	 */
	constructor(dainClient: DainClient, parallelSync?: boolean) {
	...
	public async addReferrer(authority: string, referrer?: string) {
		if (referrer) {
			...
			const buffer = (
				await this.dainClient.connection.getAccountInfo(
	...
	public async syncAll(): Promise<void> {
		const rpcRequestArgs = [
			...
			{
				commitment: this.dainClient.opts.commitment,
		...
		const rpcJSONResponse: any =
			// @ts-ignore
			await this.dainClient.connection._rpcRequest(
				'getProgramAccounts',
				rpcRequestArgs
			);
