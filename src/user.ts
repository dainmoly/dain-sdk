...
import { HealthComponent, HealthComponents, MarginCategory, MarketType, OraclePriceData, Order, PerpMarketAccount, PerpPosition, PositionDirection, SpotBalanceType, SpotMarketAccount, SpotPosition, UserAccount, UserStatsAccount, UserStatus } from "./types";
import { calculateAssetWeight, calculateBaseAssetValue, calculateBaseAssetValueWithOracle, calculateCollateralDepositRequiredForTrade, calculateEntryPrice, calculateLiabilityWeight, calculateLiveOracleTwap, calculateMarginUSDCRequiredForTrade, calculateMarketMarginRatio, calculateMarketOpenBidAsk, calculatePerpLiabilityValue, calculatePositionFundingPNL, calculatePositionPNL, calculateReservePrice, calculateSpotMarketMarginRatio, calculateUnrealizedAssetWeight, calculateWeightedTokenValue, calculateWithdrawLimit, calculateWorstCaseBaseAssetAmount, calculateWorstCasePerpLiabilityValue, divCeil, getBalance, getPerpMarketTierNumber, getSignedTokenAmount, getSpotMarketTierNumber, getStrictTokenValue, getTokenAmount, getTokenValue, getUser30dRollingVolumeEstimate, getWorstCaseTokenAmounts, isSpotPositionAvailable, isVariant, positionIsAvailable, sigNum, standardizeBaseAssetAmount } from "./modules";
import BN from "bn.js";
import { AMM_RESERVE_PRECISION, AMM_RESERVE_PRECISION_EXP, AMM_TO_QUOTE_PRECISION_RATIO, BASE_PRECISION, BN_MAX, DUST_POSITION_SIZE, FIVE_MINUTE, MARGIN_PRECISION, ONE, OPEN_ORDER_MARGIN_REQUIREMENT, PRICE_PRECISION, QUOTE_PRECISION, QUOTE_PRECISION_EXP, QUOTE_SPOT_MARKET_INDEX, SPOT_MARKET_WEIGHT_PRECISION, TEN, TEN_THOUSAND, TWO, ZERO } from "./constants";
import { StrictOraclePrice } from "./oracles/strictOraclePrice";
import { BigNum } from "./modules/bigNum";


export class User {
  readonly client: DainClient;
  ...
  public constructor(client: DainClient,
    ...
    this.client = client;
    this.userAccountPublicKey = userPublicKey;
    this.userAccount = userAccount;

    this.userStatsAccountPublicKey = userStatsPublicKey;
  ...
  public getUserStatsAccount(): UserStatsAccount {
    return this.userStatsAccount;
  }
  ...
  /**
   * Gets the user's current position for a given perpMarket. If the user has no position returns undefined
   * @param marketIndex
  ...
  /**
   * Gets the user's current position for a given spotMarket. If the user has no position returns undefined
   * @param marketIndex
  ...
  /**
   * Returns the token amount for a given market. The spotMarket precision is based on the token mint decimals.
  ...
  public getOpenOrdersForUserAccount(userAccount?: UserAccount): Order[] {
    return userAccount ?
      userAccount.orders.filter((order) =>
  ...
  public async exists(): Promise<boolean> {
    const userAccountRPCResponse =
      await this.client.connection.getParsedAccountInfo(
  ...
  /**
   * calculates the total open bids/asks in a perpMarket (including lps)
   * @returns : open bids
   * @returns : open asks
   */
  ...
  public getLPBidAsks(marketIndex: number, lpShares?: BN): [BN, BN] {
    ...
    const market = this.client.getPerpMarketAccount(marketIndex) as PerpMarketAccount;
  ...
  public getPerpPositionWithLPSettle(
    ...
    const market = this.client.getPerpMarketAccount(position.marketIndex) as PerpMarketAccount;
    ...
    if (burnLpShares && position.remainderBaseAssetAmount != 0) {
      const oraclePriceData = this.client.getOracleDataForPerpMarket(
  ...
  public getPerpBuyingPower(marketIndex: number, collateralBuffer = ZERO): BN {
    ...
    const worstCaseBaseAssetAmount = perpPosition
      ? calculateWorstCaseBaseAssetAmount(
        perpPosition,
        perpMarket as PerpMarketAccount,
  ...
  getPerpBuyingPowerFromFreeCollateralAndBaseAssetAmount(
    ...
    const marginRatio = calculateMarketMarginRatio(
      this.client.getPerpMarketAccount(marketIndex) as PerpMarketAccount,
      baseAssetAmount,
      'Initial',
  ...
  public getMaintenanceMarginRequirement(): BN {
    ...
    if (this.isBeingLiquidated()) {
      liquidationBuffer = new BN(
        this.client.getStateAccount().liquidationMarginBufferRatio
      );
    }
  ...
  public getUnrealizedPNL(
    ...
    return this.getActivePerpPositions()
      ...
      .reduce((unrealizedPnl, perpPosition) => {
        const market = this.client.getPerpMarketAccount(
        ...
        const quoteSpotMarket = this.client.getSpotMarketAccount(
  ...
  public getSpotMarketAssetAndLiabilityValue(
    ...
    for (const spotPosition of this.getUserAccount().spotPositions) {
      ...
      const spotMarketAccount =
        this.client.getSpotMarketAccount(spotPosition.marketIndex) as SpotMarketAccount;
  ...
  calculateWeightedPerpPositionLiability(
    ...
    if (marginCategory) {
      ...
      const quoteOraclePriceData = this.client.getOracleDataForSpotMarket(
  ...
  /**
   * calculates position value of a single perpMarket in margin system
   * @returns : Precision QUOTE_PRECISION
   */
  ...
  public getPositionEstimatedExitPriceAndPnl(
    ...
    const market = this.client.getPerpMarketAccount(position.marketIndex);
  ...
  isDustDepositPosition(spotMarketAccount: SpotMarketAccount): boolean {
    ...
    if (!spotPosition || isSpotPositionAvailable(spotPosition)) {
      return false;
    }
  ...
  getSpotMarketAccountsWithDustPosition() {
    const spotMarketAccounts = this.client.getSpotMarketAccounts();
   ...
   *
   * Net withdraws + Net spotMarket value + Net unrealized P&L -
   */
  ...
  public getMaxLeverageForPerp(
    ...
    const market = this.client.getPerpMarketAccount(perpMarketIndex);

    const marketPrice =
      this.client.getOracleDataForPerpMarket(perpMarketIndex).price;
  ...
  public spotLiquidationPrice(
    ...
    const market = this.client.getSpotMarketAccount(marketIndex);
    ...
    const perpMarketWithSameOracle = this.client
      .getPerpMarketAccounts()
      .find((market) => market.amm.oracle.equals(oracle));
    const oraclePrice =
      this.client.getOracleDataForSpotMarket(marketIndex).price;
  ...
  public liquidationPrice(
    ...
    const oracle =
      this.client.getPerpMarketAccount(marketIndex).amm.oracle;

    const oraclePrice =
      this.client.getOracleDataForPerpMarket(marketIndex).price;
    ...
    const spotMarketWithSameOracle = this.client
  ...
  calculateEntriesEffectOnFreeCollateral(
    ...
    if (!estimatedEntryPrice.eq(ZERO) && !positionBaseSizeChange.eq(ZERO)) {
      ...
      const takerFeeTier =
        this.client.getStateAccount().perpFeeStructure.feeTiers[0];
  ...
  calculateFreeCollateralDeltaForSpot(
    ...
    if (signedTokenAmount.gt(ZERO)) {
      const oracleData = this.client.getOracleDataForSpotMarket(market.marketIndex);
  ...
  public getMaxTradeSizeUSDCForPerp(
    ...
    const marketAccount =
      this.client.getPerpMarketAccount(targetMarketIndex);
    ...
    if (maxPositionSize.gte(ZERO)) {
      ...
      if (!targetingSameSide) {
        const market = this.client.getPerpMarketAccount(targetMarketIndex);
  ...
  public getMaxTradeSizeUSDCForSpot(
    ...
    const market = this.client.getSpotMarketAccount(targetMarketIndex);

    const oracle = this.client.getOracleDataForSpotMarket(targetMarketIndex);
  ...
  public getMaxSwapAmount({
    ...
    const inMarket = this.client.getSpotMarketAccount(inMarketIndex);

    const outMarket = this.client.getSpotMarketAccount(outMarketIndex);
  ...
  calculateSpotPositionFreeCollateralContribution(
    ...
    const spotMarketAccount = this.client.getSpotMarketAccount(spotPosition.marketIndex) as SpotMarketAccount;
  ...
  public accountLeverageAfterSwap({
    ...
    const inMarket = this.client.getSpotMarketAccount(inMarketIndex) as SpotMarketAccount;
    const outMarket = this.client.getSpotMarketAccount(outMarketIndex) as SpotMarketAccount;
  ...
  public accountLeverageRatioAfterTrade(
    ...
    const perpMarket = this.client.getPerpMarketAccount(targetMarketIndex);
  ...
  public getUserFeeTier(marketType: MarketType, now?: BN) {
    const state = this.client.getStateAccount();

    let feeTierIndex = 0;
    if (isVariant(marketType, 'perp')) {
      const userStatsAccount: UserStatsAccount = this.userStatsAccount;
  ...
  public calculateFeeForQuoteAmount(quoteAmount: BN, marketIndex?: number): BN {
    if (marketIndex !== undefined) {
      const takerFeeMultiplier = this.client.getMarketFees(
        MarketType.PERP,
        marketIndex,
        this
      ).takerFee;
  ...
  /**
   * Calculates a user's max withdrawal amounts for a spotMarket. If reduceOnly is true,
  ...
  public getWithdrawalLimit(marketIndex: number, reduceOnly?: boolean): BN {
    ...
    const spotMarket = this.client.getSpotMarketAccount(marketIndex) as SpotMarketAccount;
    ...
    for (const perpPosition of this.getActivePerpPositions()) {
      safestPerpTier = Math.min(
        ...
        getPerpMarketTierNumber(
          this.client.getPerpMarketAccount(perpPosition.marketIndex) as PerpMarketAccount
        )
      );
    }
  ...
  public getPerpPositionHealth({
    ...
    const perpMarket = this.client.getPerpMarketAccount(perpPosition.marketIndex) as PerpMarketAccount;
    const _oraclePriceData =
      oraclePriceData ||
      this.client.getOracleDataForPerpMarket(perpMarket.marketIndex);
    ...
    const _quoteOraclePriceData = quoteOraclePriceData || this.client.getOracleDataForSpotMarket(QUOTE_SPOT_MARKET_INDEX);
  ...
  public getHealthComponents({
    ...
    let netQuoteValue = ZERO;
    for (const spotPosition of this.getActiveSpotPositions()) {
      const spotMarketAccount = this.client.getSpotMarketAccount(spotPosition.marketIndex);
    ...
    if (!netQuoteValue.eq(ZERO)) {
      const spotMarketAccount = this.client.getSpotMarketAccount(QUOTE_SPOT_MARKET_INDEX);

      const oraclePriceData = this.getOracleDataForSpotMarket(QUOTE_SPOT_MARKET_INDEX);
  ...
  private getOracleDataForPerpMarket(marketIndex: number): OraclePriceData {
    return this.client.getOracleDataForPerpMarket(marketIndex);
  }

  private getOracleDataForSpotMarket(marketIndex: number): OraclePriceData {
    return this.client.getOracleDataForSpotMarket(marketIndex);
  }
}