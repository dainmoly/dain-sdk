...
export class User {
  ...
  public constructor(config: UserConfig) {
    ...
    if (config.accountSubscription?.type === "polling") {
      this.accountSubscriber = new PollingUserAccountSubscriber(
        config.dainClient.connection,
        config.userAccountPublicKey,
        config.accountSubscription.accountLoader,
        this.dainClient.program.account.user.coder.accounts.decodeUnchecked.bind(
  ...
  public getTokenAmount(marketIndex: number): BN {
    ...
    const spotMarket = this.dainClient.getSpotMarketAccount(marketIndex);
  ...
  public async exists(): Promise<boolean> {
    const userAccountRPCResponse =
      await this.dainClient.connection.getParsedAccountInfo(
  ...
  public getLPBidAsks(marketIndex: number, lpShares?: BN): [BN, BN] {
    ...
    const market = this.dainClient.getPerpMarketAccount(marketIndex);
  ...
  public getPerpPositionWithLPSettle(
    ...
    const market = this.dainClient.getPerpMarketAccount(position.marketIndex);
    ...
    if (burnLpShares && position.remainderBaseAssetAmount != 0) {
      const oraclePriceData = this.dainClient.getOracleDataForPerpMarket(
  ...
  public getPerpBuyingPower(marketIndex: number, collateralBuffer = ZERO): BN {
    ...
    const perpMarket = this.dainClient.getPerpMarketAccount(marketIndex);
  ...
  getPerpBuyingPowerFromFreeCollateralAndBaseAssetAmount(
    ...
    const marginRatio = calculateMarketMarginRatio(
      this.dainClient.getPerpMarketAccount(marketIndex),
      baseAssetAmount,
      "Initial",
  ...
  public getMaintenanceMarginRequirement(): BN {
    ...
    if (this.isBeingLiquidated()) {
      liquidationBuffer = new BN(
        this.dainClient.getStateAccount().liquidationMarginBufferRatio
      );
    }
  ...
  public getUnrealizedPNL(
    ...
    return this.getActivePerpPositions()
      ...
      .reduce((unrealizedPnl, perpPosition) => {
        const market = this.dainClient.getPerpMarketAccount(
        ...
        const quoteSpotMarket = this.dainClient.getSpotMarketAccount(
  ...
  public getFuelBonus(
    ...
    const userStats = this.dainClient.getUserStats();
      ...
      if (fuelBonusNumerator.gt(ZERO)) {
        for (const spotPosition of this.getActiveSpotPositions()) {
          const spotMarketAccount: SpotMarketAccount =
            this.dainClient.getSpotMarketAccount(spotPosition.marketIndex);
        ...
        for (const perpPosition of this.getActivePerpPositions()) {
          ...
          const perpMarketAccount = this.dainClient.getPerpMarketAccount(
            perpPosition.marketIndex
          );
  ...
  calculateWeightedPerpPositionLiability(
    ...
    if (marginCategory) {
      ...
      const quoteOraclePriceData = this.dainClient.getOracleDataForSpotMarket(
        QUOTE_SPOT_MARKET_INDEX
      );
  ...
  getSpotMarketAccountsWithDustPosition() {
    const spotMarketAccounts = this.dainClient.getSpotMarketAccounts();
  ...
  public getMaxLeverageForPerp(
    ...
    const market = this.dainClient.getPerpMarketAccount(perpMarketIndex);
    const marketPrice =
      this.dainClient.getOracleDataForPerpMarket(perpMarketIndex).price;
  ...
  public spotLiquidationPrice(
    ...
    const market = this.dainClient.getSpotMarketAccount(marketIndex);
    ...
    const perpMarketWithSameOracle = this.dainClient
      .getPerpMarketAccounts()
      .find((market) => market.amm.oracle.equals(oracle));
    const oraclePrice =
      this.dainClient.getOracleDataForSpotMarket(marketIndex).price;
  ...
  public liquidationPrice(
    ...
    const oracle = this.dainClient.getPerpMarketAccount(marketIndex).amm.oracle;

    const oraclePrice =
      this.dainClient.getOracleDataForPerpMarket(marketIndex).price;
    ...
    const spotMarketWithSameOracle = this.dainClient
  ...
  calculateEntriesEffectOnFreeCollateral(
    ...
    if (!estimatedEntryPrice.eq(ZERO) && !positionBaseSizeChange.eq(ZERO)) {
      ...
      const takerFeeTier =
        this.dainClient.getStateAccount().perpFeeStructure.feeTiers[0];
  ...
  calculateFreeCollateralDeltaForSpot(
    ...
    if (signedTokenAmount.gt(ZERO)) {
      const assetWeight = calculateAssetWeight(
        signedTokenAmount,
        this.dainClient.getOracleDataForSpotMarket(market.marketIndex).price,
        market,
        marginCategory
      );
  ...
  public getMaxTradeSizeUSDCForPerp(
    ...
    const marketAccount =
      this.dainClient.getPerpMarketAccount(targetMarketIndex);
    ...
    if (maxPositionSize.gte(ZERO)) {
      ...
      if (!targetingSameSide) {
        const market = this.dainClient.getPerpMarketAccount(targetMarketIndex);
  ...
  public getMaxTradeSizeUSDCForSpot(
    ...
    const market = this.dainClient.getSpotMarketAccount(targetMarketIndex);
    const oraclePrice =
      this.dainClient.getOracleDataForSpotMarket(targetMarketIndex).price;
  ...
  public getMaxSwapAmount({
    ...
    const inMarket = this.dainClient.getSpotMarketAccount(inMarketIndex);
    const outMarket = this.dainClient.getSpotMarketAccount(outMarketIndex);
  ...
  public accountLeverageRatioAfterTrade(
    ...
    const perpMarket = this.dainClient.getPerpMarketAccount(targetMarketIndex);
    const oracleData = this.getOracleDataForPerpMarket(targetMarketIndex);
  ...
  public getUserFeeTier(marketType: MarketType, now?: BN) {
    const state = this.dainClient.getStateAccount();
    ...
    if (isVariant(marketType, "perp")) {
      ...
      const userStatsAccount: UserStatsAccount = this.dainClient
        .getUserStats()
        .getAccount();
  ...
  public calculateFeeForQuoteAmount(quoteAmount: BN, marketIndex?: number): BN {
    if (marketIndex !== undefined) {
      const takerFeeMultiplier = this.dainClient.getMarketFees(
        MarketType.PERP,
        marketIndex,
        this
      ).takerFee;
    ...
    for (const perpPosition of this.getActivePerpPositions()) {
      safestPerpTier = Math.min(
        ...
        getPerpMarketTierNumber(
          this.dainClient.getPerpMarketAccount(perpPosition.marketIndex)
        )
      );
    }
  ...
  public getPerpPositionHealth({
    ...
    const perpMarket = this.dainClient.getPerpMarketAccount(
      perpPosition.marketIndex
    );
    const _oraclePriceData =
      oraclePriceData ||
      this.dainClient.getOracleDataForPerpMarket(perpMarket.marketIndex);
    ...
    const _quoteOraclePriceData =
      quoteOraclePriceData ||
      this.dainClient.getOracleDataForSpotMarket(QUOTE_SPOT_MARKET_INDEX);
  ...
  public getHealthComponents({
    ...
    if (!netQuoteValue.eq(ZERO)) {
      const spotMarketAccount = this.dainClient.getQuoteSpotMarketAccount();
      const oraclePriceData = this.getOracleDataForSpotMarket(
        QUOTE_SPOT_MARKET_INDEX
      );
  ...
  private getOracleDataForPerpMarket(marketIndex: number): OraclePriceData {
    return this.dainClient.getOracleDataForPerpMarket(marketIndex);
  }

  private getOracleDataForSpotMarket(marketIndex: number): OraclePriceData {
    return this.dainClient.getOracleDataForSpotMarket(marketIndex);
  }
}
