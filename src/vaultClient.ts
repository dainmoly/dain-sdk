...
import { DriftVaults } from "./vault/drift_vaults";
import {
  AccountMeta,
...
import { getOrCreateATAInstruction } from "./vault/utils";
...
export class VaultClient {
  ...
  public async calculateVaultEquityInDepositAsset(params: {
    ...
    const spotMarket = this.dainClient.getSpotMarketAccount(
      vaultAccount.spotMarketIndex
    );
    const spotOracle = this.dainClient.getOracleDataForSpotMarket(
      vaultAccount.spotMarketIndex
    );
  ...
  public async initializeVault(
    ...
    const driftState = await this.dainClient.getStatePublicKey();
    const spotMarket = this.dainClient.getSpotMarketAccount(
      params.spotMarketIndex
    );
    if (!spotMarket) {
      throw new Error(
        `Spot market ${params.spotMarketIndex} not found on dainClient`
      );
    }
    ...
    const accounts = {
      ...
      vault,
      tokenAccount,
      driftProgram: this.dainClient.program.programId,
    };
    ...
    if (vaultProtocolParams) {
      ...
      if (this.cliMode) {
        ...
        const uiAuthority = this.dainClient.wallet.publicKey;
  ...
  public async updateDelegate(
    ...
    if (this.cliMode) {
      ...
      const updateDelegateIx = await this.program.methods
        .updateDelegate(delegate)
        .accounts({ ...accounts, manager: this.dainClient.wallet.publicKey })
        .instruction();
  ...
  public async updateMarginTradingEnabled(
    ...
    try {
      remainingAccounts = this.dainClient.getRemainingAccounts({
  ...
  private async handleWSolMovement(
    ...
    if (isSolDeposit) {
      const { ixs: createWSolAccountIxs, pubkey } =
        await this.dainClient.getWrappedSolAccountCreationIxs(amount, true);
      ...
      postIxs.push(
        createCloseAccountInstruction(
          userTokenAccount,
          this.dainClient.wallet.publicKey,
          this.dainClient.wallet.publicKey,
          []
        )
      );
  ...
  public async managerDeposit(
    ...
    const vaultAccount = await this.program.account.vault.fetch(vault);
    const driftSpotMarket = this.dainClient.getSpotMarketAccount(
      vaultAccount.spotMarketIndex
    );
    if (!driftSpotMarket) {
      throw new Error(
        `Spot market ${vaultAccount.spotMarketIndex} not found on dainClient`
      );
    }

    const user = await this.getSubscribedVaultUser(vaultAccount.user);

    const remainingAccounts = this.dainClient.getRemainingAccounts({
      userAccounts: [user.getUserAccount()],
    ...
    const accounts = {
      ...
      driftProgram: this.dainClient.program.programId,
      driftState: await this.dainClient.getStatePublicKey(),
    ...
    if (this.cliMode) {
      ...
      const managerDepositIx = await this.program.methods
        ...
        .accounts({
          ...accounts,
          userTokenAccount,
          manager: this.dainClient.wallet.publicKey,
        })
        .remainingAccounts(remainingAccounts)
        .instruction();
      return await this.createAndSendTxn(
  ...
  public async managerRequestWithdraw(
    ...
    if (!this.dainClient.wallet.publicKey.equals(vaultAccount.manager)) {
  ...
  public async managerWithdraw(
    ...
    const ix = this.program.instruction.managerWithdraw({
      accounts: {
        ...
        driftSigner: this.dainClient.getStateAccount().signer,
        tokenProgram: TOKEN_PROGRAM_ID,
      },
      remainingAccounts,
    });
  ...
  private createInitVaultDepositorIx(
    ...
    const vaultDepositor = getVaultDepositorAddressSync(
      this.program.programId,
      vault,
      authority || this.dainClient.wallet.publicKey
    );

    const accounts = {
      vaultDepositor,
      vault,
      authority: authority || this.dainClient.wallet.publicKey,
    };

    const initIx = this.program.instruction.initializeVaultDepositor({
      accounts: {
        ...accounts,
        payer: payer || authority || this.dainClient.wallet.publicKey,
  ...
  public async initializeTokenizedVaultDepositor(params: {
    ...
    if (params.decimals === undefined || params.sharesBase === undefined) {
      ...
      const spotMarketAccount = this.dainClient.getSpotMarketAccount(
        vault.spotMarketIndex
      );
      if (!spotMarketAccount) {
        throw new Error(
          `DainClient failed to load vault's spot market (marketIndex: ${vault.spotMarketIndex})`
        );
      }
    ...
    const accounts = {
      ...
      payer: this.dainClient.wallet.publicKey,
    };
  ...
  public async createTokenizeSharesIx(
    ...
    const userAtaExists = await this.dainClient.connection.getAccountInfo(
      userAta
    );
  ...
  public async tokenizeShares(
    ...
    if (this.cliMode) {
      try {
        ...
        const txSig = await this.dainClient.txSender.send(
          tx,
          undefined,
          undefined,
          false
  ...
  public async prepDepositTx(
    ...
    const driftStateKey = await this.dainClient.getStatePublicKey();
  ...
  public async deposit(
    ...
    if (this.cliMode) {
      ...
      return this.sendTxn(depositTxn, false);
    }
  }
  ...
  public async withdraw(
    ...
    if (isSolMarket) {
      const { ixs, pubkey } =
        await this.dainClient.getWrappedSolAccountCreationIxs(ZERO, false);
    ...
    if (this.cliMode) {
      ...
      return await this.createAndSendTxn(ixs, {
        computeUnits:
          (txParams?.computeUnits ?? 650_000) + (creationIxs > 0 ? 200_000 : 0),
        ...txParams,
      });
    }
  }
  ...
  public async createTxn(
    ...
    const ixs = [
      ComputeBudgetProgram.setComputeUnitLimit({
        units: txParams?.computeUnits ?? 400_000,
      }),
      ComputeBudgetProgram.setComputeUnitPrice({
        microLamports: txParams?.computeUnitsPrice ?? 1_000_000,
      }),
      ...vaultIxs,
    ];

    return (await this.dainClient.txHandler.buildTransaction({
      connection: this.dainClient.connection,
      instructions: ixs,
      ...
      fetchMarketLookupTableAccount:
        this.dainClient.fetchMarketLookupTableAccount.bind(this.dainClient),
  ...
  public async sendTxn(
    ...
    if (simulateTransaction) {
      try {
        const resp = await this.dainClient.connection.simulateTransaction(
          transaction,
          {
            sigVerify: false,
            commitment: this.dainClient.connection.commitment,
          }
        );
      ...
      const resp = await this.dainClient.sendTransaction(
        transaction,
        [],
        this.dainClient.opts
      );
      if (resp.txSig !== txSig) {
        console.error(
  ...
  public async createAndSendTxn(
    ...
    const txSig = await this.sendTxn(tx, false);
  ...
  public async initializeInsuranceFundStake(
    ...
    const spotMarket = this.dainClient.getSpotMarketAccount(spotMarketIndex);
    if (!spotMarket) {
      throw new Error(`Spot market ${spotMarketIndex} not found on dainClient`);
    }
  ...
  public async addToInsuranceFundStake(
    ...
    if (!vaultAccount.manager.equals(this.dainClient.wallet.publicKey)) {
      throw new Error(
  ...
  public async protocolRequestWithdraw(
    ...
    if (!this.dainClient.wallet.publicKey.equals(vpAccount.protocol)) {
    ...
    const oracleFeedsToCrankIxs: TransactionInstruction[] = oracleFeedsToCrank
      ? ((await Promise.all(
          oracleFeedsToCrank.map(async (feedConfig) => {
            ...
            return this.dainClient.getPostSwitchboardOnDemandUpdateAtomicIx(
              feedConfig.feed
            );
          })
