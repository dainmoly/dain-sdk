...
import {
  ...
  DainClientAccountEvents,
  DainClientAccountSubscriber,
} from "./accounts/types";
...
import { WebSocketClientAccountSubscriber } from "./accounts/webSocketClientAccountSubscriber";
...
export class DainClient {
  ...
  accountSubscriber: DainClientAccountSubscriber;
  eventEmitter: StrictEventEmitter<EventEmitter, DainClientAccountEvents>;
  ...
  public constructor(config: DainClientConfig) {
    ...
    if (config.accountSubscription?.type === "polling") {
      this.accountSubscriber = new PollingClientAccountSubscriber(
        this.program,
  ...
  public getUser(subAccountId?: number, authority?: PublicKey): User {
    ...
    if (!this.users.has(userMapKey)) {
      throw new Error(`DainClient has no user for user id ${userMapKey}`);
    }
  ...
  public triggerEvent(eventName: keyof DainClientAccountEvents, data?: any) {
   ...
   * @param opts :: Will fallback to DainClient's opts if not provided
   * @param preSigned
   * @returns
   */
  sendTransaction(
