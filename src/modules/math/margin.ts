...
export function calculateMarginUSDCRequiredForTrade(
	...
	const targetMarket = client.getPerpMarketAccount(targetMarketIndex) as PerpMarketAccount;

	const price =
		entryPrice ??
		client.getOracleDataForPerpMarket(targetMarket.marketIndex).price;
...
export function calculateCollateralDepositRequiredForTrade(
	...
	const collateralMarket = client.getSpotMarketAccount(collateralIndex) as SpotMarketAccount;

	const collateralOracleData =
		client.getOracleDataForSpotMarket(collateralIndex);
	...
	const baseAmountRequired = client
