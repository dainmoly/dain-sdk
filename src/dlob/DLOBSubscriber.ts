...
export class DLOBSubscriber {
	...
	public getL2({
		...
		if (marketName) {
			const derivedMarketInfo =
				this.dainClient.getMarketIndexAndType(marketName);
		...
		if (isPerp) {
			...
			oraclePriceData = this.dainClient.getOracleDataForPerpMarket(
				perpMarketAccount.marketIndex
			);
		} else {
			oraclePriceData =
				this.dainClient.getOracleDataForSpotMarket(marketIndex);
		}
		...
		if (isPerp && includeVamm) {
			...
			fallbackL2Generators = [
				getVammL2Generator({
					marketAccount: this.dainClient.getPerpMarketAccount(marketIndex),
					oraclePriceData,
	...
	public getL3({
		...
		if (isPerp) {
			oraclePriceData =
				this.dainClient.getOracleDataForPerpMarket(marketIndex);
		} else {
			oraclePriceData =
				this.dainClient.getOracleDataForSpotMarket(marketIndex);
		}
