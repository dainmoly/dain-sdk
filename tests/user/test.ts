...
async function makeMockUser(
	...
	mockUser.dainClient._isSubscribed = true;
	mockUser.dainClient.accountSubscriber.isSubscribed = true;
	...
	mockUser.dainClient.getPerpMarketAccount = getMockPerpMarket;
	mockUser.dainClient.getSpotMarketAccount = getMockSpotMarket;
	mockUser.dainClient.getOraclePriceDataAndSlot = getMockOracle;
	mockUser.dainClient.getOracleDataForPerpMarket = getOracleDataForPerpMarket;
	mockUser.dainClient.getOracleDataForSpotMarket = getOracleDataForSpotMarket;
	return mockUser;
}
...
describe('User Tests', () => {
	...
	it('user account unsettled pnl', async () => {
		...
		const quotePrice = mockUser.dainClient.getOracleDataForSpotMarket(0).price;
		...
		const pnl1 = calculatePositionPNL(
			...
			activePerps[0],
			false,
			mockUser.dainClient.getOracleDataForPerpMarket(0)
		);
