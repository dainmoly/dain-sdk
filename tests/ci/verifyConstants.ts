...
describe('Verify Constants', function () {
	...
	const devnetDainClient = new DainClient({
	...
	const mainnetDainClient = new DainClient({
	...
	before(async () => {
		await devnetDainClient.subscribe();
		await mainnetDainClient.subscribe();

		const lookupTable =
			await mainnetDainClient.fetchMarketLookupTableAccount();
		lutAccounts = lookupTable.state.addresses.map((x) => x.toBase58());
	});

	after(async () => {
		await devnetDainClient.unsubscribe();
		await mainnetDainClient.unsubscribe();
	});

	it('has all mainnet markets', async () => {
		const spotMarkets = mainnetDainClient.getSpotMarketAccounts();
		...
		const perpMarkets = mainnetDainClient.getPerpMarketAccounts();
	...
	it('has all devnet markets', async () => {
		const spotMarkets = devnetDainClient.getSpotMarketAccounts();
		spotMarkets.sort((a, b) => a.marketIndex - b.marketIndex);
		...
		const perpMarkets = devnetDainClient.getPerpMarketAccounts();
		perpMarkets.sort((a, b) => a.marketIndex - b.marketIndex);
