import { DainClient, BulkAccountLoader } from '../../src';
...
describe('Verify IDL', function () {
	...
	it('verify idl', async () => {
		const onChainIdl = await Program.fetchIdl(
			mainnetDainClient.program.programId,
			mainnetDainClient.provider
		);

		if (onChainIdl === null) {
			throw new Error(
				`onChainIdl for ${mainnetDainClient.program.programId.toBase58()} null`
			);
		}
