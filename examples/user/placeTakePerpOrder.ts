import { BASE_PRECISION, DainClient, decodeName, NodeWallet, OptionalOrderParams, OrderType, PositionDirection, QUOTE_PRECISION } from "dain-sdk";
...
(async () => {
    ...
    const market = await client.getPerpMarketAccount(marketIndex);
    console.log(`${decodeName(market.name)} loaded`);
    ...
    const tx = await client.placeAndTakePerpOrder(
        orderParams,
    );
